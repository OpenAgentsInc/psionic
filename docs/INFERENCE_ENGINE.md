...
- The `qwen38` CPU/CUDA text-serving lane is `implemented_early`; the wider
  ...
  its published NFC and per-code-point numeric pretokenizer. R5 adds native
  storage support for the concrete GGML families in the materialized GGUFs,
  converter-layout parity against sampled BF16 rows, tensor inventories,
  MTP-skip receipts, and memory-envelope decisions. The primary Dynamic V3
  ...
  R7 is `implemented`. Retained greedy and bounded-sampling rows on an idle
  RTX 4080 bind exact prompt token `[9419]`, deterministic output IDs, zero
...
- The independent `qwen38` native vision lane is `partial`. Current `main`
  ...
  CPU and CUDA Qwen3.8 decoders have explicit multimodal generation entry
  points that consume this plan, replace token embeddings during prefill,
