...
the real decoder shape.

A retained real-shape row now sits beside it.
`fixtures/qwen38/adapters/qwen38_lm_head_lora_real_27b_v1.safetensors` is a
rank-1 LM-head LoRA artifact at the real decoder shape, generated by
`qwen38_adapter_serving_evidence` against the qualified Q4_K_M GGUF and
retained through `fixtures/qwen38/reports/qwen38_adapter_serving_evidence_v1.json`
with `scripts/run-qwen38-adapter-serving-evidence.sh` and
`scripts/check-qwen38-adapter-serving.sh`. The driver captures the real
decoder hidden state and logits, seeds a deterministic recipe that moves the
greedy argmax to the runner-up token, executes one admitted AdamW step so
`trained_step` is honest, exports the artifact bound to the declared base
identity, and serves the overlay on the native CPU runtime. The report
records baseline and adapted token IDs, exact provenance binding, and the
merged-residency, drifted-binding, and detached-binding refusals. The recipe
is disclosed in the report: this row proves real-shape artifact and serving
compatibility, not trained-model quality.

This tranche does not admit real-checkpoint training, native CPU/CUDA/Metal
backward execution, decoder-layer or vision targets, CUDA/Metal adapter
execution, merged residency, MTP-composed adapters, real/native checkpoint
recovery, evaluation gains, or promotion. Those remain required before R12
