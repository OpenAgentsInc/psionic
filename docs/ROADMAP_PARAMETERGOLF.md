...
| ID | Status | Proposed GitHub issue title | Description |
| --- | --- | --- | --- |
| `PGOLF-601` / [#188](https://github.com/OpenAgentsInc/psionic/issues/188) | open (2026-03-19) | `Psionic Parameter Golf: retire the remaining baseline CUDA blockers on the public submission path` | Umbrella runtime-closure issue for the remaining public CUDA baseline blockers. This stays open while the narrower execution issues below land and until the repo can honestly stop listing the current BF16, RoPE/GQA, RMSNorm, residual-mix, and Muon blockers on the actual submission path. |
...
| `PGOLF-617` / [#261](https://github.com/OpenAgentsInc/psionic/issues/261) | open (2026-03-19) | `Psionic Parameter Golf: widen public CUDA execution for RMSNorm backward graphs` | After the graph semantics exist, widen the public CUDA runtime path to execute the RMSNorm backward graph honestly instead of refusing those train-visible steps. |
| `PGOLF-618` / [#262](https://github.com/OpenAgentsInc/psionic/issues/262) | open (2026-03-19) | `Psionic Parameter Golf: add public CUDA train-time RoPE/GQA decoder-block execution` | Retire the decoder-core attention blocker by widening the public CUDA runtime or kernel path for the train-visible RoPE plus grouped-query attention block used by the baseline compact decoder. |
| `PGOLF-619` / [#263](https://github.com/OpenAgentsInc/psionic/issues/263) | open (2026-03-19) | `Psionic Parameter Golf: add public CUDA BF16 train-visible execution posture` | Make the public CUDA lane own honest BF16 train-visible tensor or kernel execution posture instead of only policy receipts and dense `f32` fallback surface. |
| `PGOLF-620` / [#264](https://github.com/OpenAgentsInc/psionic/issues/264) | open (2026-03-19) | `Psionic Parameter Golf: add public CUDA Muon optimizer execution` | Retire the remaining optimizer blocker by adding a public CUDA Muon runtime path for the baseline matrix-shaped parameter groups under the same honest runtime and accounting posture. |
...
- `PGOLF-617` -> widen the public CUDA runtime to execute RMSNorm backward graphs honestly
- `PGOLF-618` -> widen the public CUDA decoder-core path for train-time RoPE plus GQA execution
- `PGOLF-619` -> add honest BF16 train-visible public CUDA posture
- `PGOLF-620` -> add a public CUDA Muon optimizer path
