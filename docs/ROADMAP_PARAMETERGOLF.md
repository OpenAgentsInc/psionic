...
| ID | Status | Proposed GitHub issue title | Description |
| --- | --- | --- | --- |
| `PGOLF-601` / [#188](https://github.com/OpenAgentsInc/psionic/issues/188) | open (2026-03-19) | `Psionic Parameter Golf: retire the remaining baseline CUDA blockers on the public submission path` | Umbrella runtime-closure issue for the remaining public CUDA baseline blockers. This stays open while the narrower execution issues below land and until the repo can honestly stop listing the current BF16, RoPE/GQA, and Muon blockers on the actual submission path. |
