...
| ID | Status | Proposed GitHub issue title | Description |
| --- | --- | --- | --- |
| `PGOLF-601` / [#188](https://github.com/OpenAgentsInc/psionic/issues/188) | open (2026-03-19) | `Psionic Parameter Golf: retire the remaining baseline CUDA blockers on the public submission path` | Umbrella runtime-closure issue for the remaining public CUDA baseline blockers. This stays open while the narrower execution issues below land and until the repo can honestly stop listing the current BF16 and decoder-block blockers on the actual submission path. |
...
| `PGOLF-622` / [#266](https://github.com/OpenAgentsInc/psionic/issues/266) | open (2026-03-19) | `Psionic Parameter Golf: add public reverse-mode support for the RoPE/GQA decoder block` | Follow the new bounded forward decoder-block closure with an honest reverse-mode or trainer-owned gradient path so the attention-family blocker narrows from missing execution to the remaining train-graph gap. |
| `PGOLF-624` / [#268](https://github.com/OpenAgentsInc/psionic/issues/268) | open (2026-03-19) | `Psionic Parameter Golf: add public CUDA execution for decoder-block backward graphs` | Follow the decoder reverse-mode or trainer-graph seam with public CUDA backward execution so the attention-family lane does not hide the remaining runtime step inside `PGOLF-601`. |
...
- `PGOLF-622` -> follow the new decoder-core forward path with public reverse-mode or trainer-owned gradient closure
- `PGOLF-624` -> follow the decoder reverse-mode seam with public CUDA backward execution
