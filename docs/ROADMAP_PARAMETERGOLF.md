...
| `PGOLF-604` / [#194](https://github.com/OpenAgentsInc/psionic/issues/194) | open (2026-03-18) | `Psionic Parameter Golf: add a Rust-native 1xH100 baseline trainer entrypoint` | The repo now has a real Rust-native `parameter_golf_single_h100_bringup` command that binds cached FineWeb `sp1024` data, tokenizer identity, explicit local CUDA machine-admission truth, baseline model or optimizer truth, and the current CUDA blocker list into one typed report. It also now has the bounded baseline forward-logits plus seeded backward graph lane needed to train against the real Parameter Golf parameter surface without pretending the current IR owns embeddings or cross-entropy natively. The remaining work is to turn those pieces into the actual Psionic-owned single-H100 training loop instead of a refusal-bound readiness report. |
