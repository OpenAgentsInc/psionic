...
| `PGOLF-625` / [#270](https://github.com/OpenAgentsInc/psionic/issues/270) | done (2026-03-19) | `Psionic Parameter Golf: add Parameter Golf train-time SiLU graph and CUDA runtime support` | `psionic-core`, `psionic-ir`, `psionic-backend-cpu`, and `psionic-backend-cuda` now expose bounded `silu` plus `silu_backward` backend-extension ops, register them in the built-in operator surface, evaluate them on the dense reference path, materialize reverse-mode SiLU gradients, and execute the forward plus seeded backward graph on CUDA through a bounded host-orchestrated `f32` runtime. This retires the MLP activation gap on the honest Rust-native train graph without claiming logits, loss, or end-to-end trainer closure. |
| `PGOLF-626` / [#269](https://github.com/OpenAgentsInc/psionic/issues/269) | open (2026-03-19) | `Psionic Parameter Golf: lower the full baseline forward-logits and seeded backward path` | With RMSNorm, decoder-block, residual-mix, BF16/Muon, and SiLU train-visible graph/runtime seams now landed, the remaining `PGOLF-604` core gap is lowering the full baseline forward pass through logits and proving one seeded backward path over the real Parameter Golf baseline geometry before promoting the single-H100 bring-up seam into a true trainer entrypoint. |
...
- `PGOLF-618` -> landed the bounded public CUDA decoder-core forward path for train-time RoPE plus GQA execution
...
- `PGOLF-625` -> landed bounded train-time SiLU forward and backward graph/runtime support across the core IR, CPU reference backend, and CUDA runtime
- `PGOLF-626` -> lower the full baseline forward-logits and seeded backward path so the single-H100 bring-up can become a real trainer entrypoint
