...
| `PGOLF-626` / [#269](https://github.com/OpenAgentsInc/psionic/issues/269) | open (2026-03-19) | `Psionic Parameter Golf: lower the full baseline forward-logits and seeded backward path` | With RMSNorm, decoder-block, residual-mix, BF16/Muon, and the actual baseline `ReLU^2` train-visible graph/runtime seam now landed, the remaining `PGOLF-604` core gap is lowering the full baseline forward pass through logits and proving one seeded backward path over the real Parameter Golf baseline geometry before promoting the single-H100 bring-up seam into a true trainer entrypoint. |
...
- `PGOLF-625` -> landed bounded train-time SiLU forward and backward graph/runtime support across the core IR, CPU reference backend, and CUDA runtime, but that was extra pointwise coverage rather than the actual baseline MLP blocker
- `PGOLF-627` -> landed exact bounded train-time ReLU^2 forward and backward graph/runtime support across the core IR, CPU reference backend, and CUDA runtime
