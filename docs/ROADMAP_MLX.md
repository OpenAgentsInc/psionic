...
> `PMLX-703` / `#16`, `PMLX-704` / `#17`, `PMLX-705` / `#18`, and
> `PMLX-706` / `#19`, and `PMLX-707` / `#20`,
...
| `PMLX-307` / [#3852](https://github.com/OpenAgentsInc/openagents/issues/3852) | done (2026-03-16) | `Psionic MLX: add quantized module families and module-level quantize behavior` | `psionic-nn` now exposes a first eval-oriented quantized-module shell through `Module::quantize(...)`, explicit quantize reports with keep-dense versus strict posture, and `QuantizedLinear` plus `QuantizedEmbedding` wrappers over `int8_symmetric` block storage with dequantize-to-`f32` forward semantics, while keeping broader quantized training and export parity out of scope. |
...
| `PMLX-405` / [#3857](https://github.com/OpenAgentsInc/openagents/issues/3857) | done (2026-03-16) | `Psionic MLX: expose backend debug, logging, and capture hooks for Metal and CUDA lanes` | `psionic-array` now exposes a bounded public backend-debug surface above `psionic-compiler` and `psionic-runtime`, including lane-specific support matrices for `cpu` / `metal` / `cuda`, stable backend identity snapshots, retained runtime log events, bounded local-runtime observability snapshots, explicit debug captures with compiler trace and plan-debug output, and optional lane-labeled JSON capture bundles while honestly refusing vendor-native profiler claims on the current reference runtime. |
| `PMLX-406` / [#3858](https://github.com/OpenAgentsInc/openagents/issues/3858) | done (2026-03-16) | `Psionic MLX: add a custom kernel authoring and extension surface for accelerated backends` | `psionic-array` now exposes a bounded public extension-authoring layer above `psionic-ir`'s extensible operator registry, including custom-op, custom-kernel, backend-plugin, and quantizer-plugin contract registration, dispatch resolution against the current backend lane, declared-output validation for custom ops, and stable extension-registry snapshots while honestly stopping short of generic custom-op runtime execution on the current reference surface. |
...
| `PMLX-705` | done (2026-03-17) | `Psionic MLX: add an audio package for TTS, speech-to-speech, codecs, and speech model IO` | `psionic-mlx-audio` now provides a bounded MLX-style audio package with builtin `kokoro`/`xtts`/`encodec`-class family metadata, quantized-checkpoint descriptors, WAV IO, codec normalization helpers, CPU-reference text-to-speech and speech-to-speech request contracts, explicit stream chunks, and one server-facing speech request/response surface without claiming a production neural speech runtime. |
