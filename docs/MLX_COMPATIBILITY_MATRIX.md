...
> `PMLX-607` / `#3872`, `PMLX-608` / `#3873`, and `PMLX-701` / `#14`.
...
| `mlx_package_ecosystem` | `convertible` | A first bounded local MLX-lm text package now exists in `psionic-mlx-lm`, but remote catalogs, served text APIs, multimodal/audio packages, recipe layers, and benchmark packages remain later ecosystem work. | `PMLX-702` through `PMLX-709` | The first local text package is real, but it does not imply remote registry support, OpenAI-compatible serving, multimodal/audio closure, training recipes, synthetic/publish workflows, or benchmark-package completion. |
