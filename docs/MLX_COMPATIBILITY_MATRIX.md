...
| `mlx_package_ecosystem` | `convertible` | Bounded local MLX-style text, catalog, text-serving, multimodal, audio, recipe, workflow, and benchmark packages now exist in `psionic-mlx-lm`, `psionic-mlx-catalog`, `psionic-mlx-serve`, `psionic-mlx-vlm`, `psionic-mlx-audio`, `psionic-mlx-recipes`, `psionic-mlx-workflows`, and `psionic-mlx-bench`, but the final ecosystem examples/guides layer remains later work. | `PMLX-709` | The first local text, catalog, text-serving, multimodal, audio, recipe, workflow, and benchmark packages are real, but they do not imply the full examples/guides layer. |
