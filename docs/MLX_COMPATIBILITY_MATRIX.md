...
> function-export, `.mlxfn`, public array, distributed, and naming-facade
> rows after `PMLX-601` / `#3866`, `PMLX-602` / `#3867`,
> `PMLX-603` / `#3868`, and `PMLX-606` / `#3871`.
...
| `mlx_naming_facade_and_bindings` | `convertible` | The optional `psionic-mlx-compat` crate now exposes a bounded MLX-like module layout and naming facade over supported Psionic-native core, transform, `nn`, optimizer, io, distributed, and report surfaces, but Python/C/Swift bindings and broader migration guidance remain later work. | `PMLX-607`, `PMLX-608` | This facade is a thin name and module shim over existing Psionic-native surfaces; it does not imply MLX-identical signatures, missing native semantics, or any Python, C, or Swift binding layer. |
