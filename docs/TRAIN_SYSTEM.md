...
> Status: updated 2026-04-01 after reviewing `docs/MVP.md`,
> `docs/OWNERSHIP.md`,
...
separate full-layer-smoke LoRA command documented below; exact attention, MLP,
...
DPO, GRPO rollout seed, trace-store, eval-pack, SFT shard, manifest, and
receipt artifacts from one declared source root. It records all source and
output hashes, writes separate train/dev/holdout files, rejects private or
scorer-only material from trainable files, records replay commands in the
manifest, and exposes stable `corpus_shard_id` values for Pylon worker jobs.
The trace store keeps full failed model responses and full public
model-written answer text, but blocks private answers, hidden labels,
scorer-only labels, judge-only labels, and runner-added answer text. The
recorded local smoke over
`tasks/synthetic/legal-workflow-v1/training/dpo_run_sample` wrote 40 SFT
records split into 32 train, 5 dev, and 3 holdout records; 1408 DPO pairs split
into 1145 train, 135 dev, and 128 holdout pairs; 1408 GRPO rollout seeds split
into 1109 train, 139 dev, and 160 holdout seeds; 16 trace-store records; two
Pylon shard refs; trace-store hash
...
search, and supervised fine-tune refresh work. The hillclimb loop is now
controlled by `crates/psionic-train/src/qwen_legal_hillclimb_controller.rs`
and `psionic-train qwen-legal-hillclimb --plan <plan.json>`. The plan declares
model id, adapter target, corpus split, Pylon count, training method,
evaluator split, promotion rule, baseline, candidate, and regression checks.
The controller appends baseline, candidate, and regression records to a local
registry with score, delta, model, adapter, data split, worker set, payment
status, and replay command, then exports an Autopilot4 progress feed with
recent runs and score history. Guardrails reject failed candidates, train-only
improvements presented as broad benchmark gains, training-split evaluator
claims, unsettled candidate payments, insufficient deltas, and policy-breaking
regressions before those records can become champion claims. It now also emits
an offline
...
holdout regression. The lane now also emits a phase-four perfect-score push
...
For a slightly stronger real-weight check, use the sampled projection backend:
...
Recorded local sampled-projection result:

- forward execution status: `sampled_projection`
- prompt tokens: `70`
- required text tensors admitted: `866 / 866`
- non-text tensors reported: `333`
- rows read from real weights: one `model.language_model.embed_tokens.weight`
  row and seven `lm_head.weight` rows
- input token row: `271`
- sampled candidate token rows: `0`, `1`, `2`, `3`, `4`, `5`, `271`
...
This is real checkpoint math: Psionic seeks into the downloaded BF16
...
The forward admission command now also supports a bounded full-layer smoke:
...
rows, and emits deterministic sampled logits. It is memory-bounded and
row-sparse. It proves the Rust layer order, real tensor lookup, row decoding,
layer visitation, deterministic replay, and sampled-logit report shape. It
records the command line, config hash, tokenizer hash, tensor-index hash,
prompt hash, tensor-read hashes, and sampled-logit hash. It does not claim
full-width attention, full MLP activation materialization,
full-vocabulary logits, multi-token generation, or training gradients.
Unsupported model families, missing tensors, failed text tensor admission, and
MoE/router paths still refuse plainly.

The first real-weight Qwen3.6-27B full-layer-smoke LoRA training command is:
...
instead of the old embedding-row sampled projection. The trainer validates
the dense target module list, records QLoRA compatibility metadata when
requested, keeps the base model frozen, and writes a resumable optimizer state
checkpoint. The exported adapter still uses the same `lm_head.lora_A.weight`
/ `lm_head.lora_B.weight` safetensors format already consumed by the DPO and
GRPO parent-adapter loaders. Exact backward kernels for `q_proj`, `k_proj`,
`v_proj`, `o_proj`, `gate_proj`, `up_proj`, and `down_proj` remain open work.

Use `train_type: "lora"` when the Pylon can keep the BF16 base checkpoint
available and train F32 adapter deltas. Use `train_type: "qlora"` for
lower-memory Pylon placement; this records q4-k compatibility metadata and
still emits F32 adapter deltas, while exact 4-bit base matmul kernels remain
separate runtime work. Router and MoE target names still refuse in this dense
27B SFT command.
...
closeout over the real-Qwen LoRA path. The trainer now uses the bounded
full-layer-smoke activation surface by default. It still does not prove
private Harvey performance, remote Pylon execution, or exact full-width
...
- `docs/PSION_PLUGIN_SEQUENCING_BENCHMARK.md` is the canonical third
  package-specific plugin benchmark doc for the convergence tranche and
...
- `docs/PSION_PLUGIN_RESULT_INTERPRETATION_BENCHMARK.md` is the canonical
  fifth package-specific plugin benchmark doc for the convergence tranche and
...
- `docs/PSION_RUN_OBSERVABILITY.md` is the canonical first run-observability
...
- `docs/PSION_GOOGLE_SINGLE_GPU_RUNBOOK.md` is the canonical first Google
  ...
  `crates/psionic-train/examples/psion_accelerated_reference_pilot.rs`, and a
...
- the first same-corpus flat-prefix-vs-windowed 9x9 comparison now also
  ...
  those artifacts keep the learned claim bounded while making the long-trace
  family split machine-readable: the flat-prefix family and the windowed
  family both stay at `5938` bps first-32 and `0/1` exact validation traces
...
- one explicit target set = `lm_head` only
- one explicit low-rank posture = rank `8`, alpha `16`
- one served-base compatibility binding = `gemma4:e4b@v1` plus non-empty served
  artifact digest, bounded tokenizer contract, and declared hidden width
- one exact checkpoint snapshot surface for save/resume over
