...
- the same dense split runtime path now also prefers direct worker-to-worker
  handoff between adjacent shard runtimes when both sides support it, while
  preserving host fallback and per-stage transport evidence
