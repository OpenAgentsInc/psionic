...
| `ROADMAP_CLUSTER.md` through `PSI-231` | replica routing, pipeline-parallel, layer-sharded, tensor-sharded serving, sharded manifests, cluster cache compatibility, trust publication | This is the strongest current adaptation from Prime/vLLM-class distributed serving ideas, and `PSI-242` now folds those lanes onto one canonical serving-semantics model instead of leaving clustered serving as a semantic side product. |
...
| Scheduler and queue semantics | execution-profile truth, queue-policy truth, throughput-class truth, first continuous-batching CPU scheduler, mixed prefill/decode scheduling receipts, real request-owned block/paged KV management, automatic prefix caching with tenancy and sampler boundaries, explicit prefill/decode capability contracts, colocated and KV-transfer handoff seams, separate TTFT/ITL metrics, caller-static-batch embeddings, seeded sampler, warm/load/unload lifecycle, and one canonical serving-semantics contract reused across local and clustered serving evidence | adjacent to `vLLM` concepts, but now materially realized in Psionic-owned runtime and serve code | broader serving APIs, router policy, and validation matrices still remain |
...
| Distributed serving topology | selected-device truth, topology plans, replica serving, pipeline sharding, layer sharding, tensor sharding, sharded-model manifests, and machine-checkable warm-route posture per clustered lane | mostly `vLLM`-class and Prime-adjacent serving-topology lessons | acceptance matrices and higher-level routing policy are still open |
...
- real request-owned block/paged KV runtime
...
Implemented now: `PSI-238` is materially landed in-tree too. The shared
scheduler and the GPT-OSS single-request paths now sit on a Psionic-owned
block/paged KV manager with request/session/shared-prefix owner bindings,
deterministic allocation and reclaim, oldest-page eviction support, backend
...
Implemented now: `PSI-245` is materially landed in-tree too. The generic server
now accepts first-class `tools` plus `tool_choice` contracts, supports `none`,
`auto`, `required`, and named-tool modes, validates tool arguments against
Psionic-owned JSON-schema matchers instead of hidden string parsing, and
...
| `PSI-242` | Psionic Cluster: unify local and clustered scheduler semantics for batching, cache, and warm-route truth | `psionic-cluster`, `psionic-runtime`, `psionic-serve` | `vLLM` plus `SGLang` | Status: implemented on 2026-03-14 via GitHub issue `#3547`. Added one canonical serving-semantics contract shared across local and clustered lanes, aligned cluster capability/evidence reporting with local execution-profile truth, and surfaced route-pinned vs topology-pinned warm-route posture explicitly on clustered execution receipts. | `PSI-237` through `PSI-241`, current cluster foundation |
...
| `PSI-248` | Psionic Adapters: add LoRA and adapter import, merge/unmerge, batching, and hosted serving | `psionic-models`, `psionic-serve`, backend crates | `SGLang` plus `llama.cpp` | Status: implemented on 2026-03-14 via GitHub issue `#3553`. Dense CPU GGUF decoder families now support truthful LM-head LoRA import from safetensors, explicit attach/detach plus merge/unmerge serving modes, compatibility/refusal reporting, and real adapter-backed generation instead of metadata-only adapter parsing or silent base-model fallback. | `PSI-233`, `PSI-244`, `PSI-245` |

### Phase 4: Router And Gateway Layer

| Local ID | Proposed GitHub issue title | Scope | Primary reference | Description | Depends on |
| --- | --- | --- | --- | --- | --- |
| `PSI-249` | Psionic Router: introduce a dedicated multi-model routing and policy crate | new `psionic-router` crate plus `psionic-serve` | `SGLang` | Status: implemented on 2026-03-14 via GitHub issue `#3554`. Added a real `psionic-router` crate with worker/model inventory, capability filters, deterministic route selection, and explicit non-goals, then wired the generic Psionic server through that routing API instead of local alias-map glue. | `PSI-243`, `PSI-247` |
| `PSI-250` | Psionic Router: add cache-aware, warm-aware, and power-of-two request placement policies | `psionic-router`, `psionic-cluster` | `SGLang` | Status: implemented on 2026-03-14 via GitHub issue `#3555`. Added explicit warm-state and safe cache-affinity inventory, bounded power-of-two least-loaded selection over warm/cache-matched candidate pools, per-route placement metrics, and generic-server route headers plus response-state worker preference so warm-aware routing is inspectable instead of implicit. | `PSI-249`, `PSI-239`, `PSI-242` |
| `PSI-251` | Psionic Router: add retries, circuit breakers, rate limiting, request queues, and worker health control | `psionic-router` | `SGLang` | Status: implemented on 2026-03-14 via GitHub issue `#3556`. Added a router-owned reliability controller with retry/refusal policy, queue depth control, rate limiting, circuit-breaker state, health gating, and explicit per-worker metrics or traces exercised through failure-injection tests. | `PSI-249` |
| `PSI-252` | Psionic Router: add pluggable response and conversation storage backends | `psionic-router` | `SGLang` | Status: implemented on 2026-03-14 via GitHub issue `#3557`. Moved response and conversation persistence into `psionic-router` behind a pluggable backend trait, added bounded in-memory plus best-effort local JSON durability backends with explicit retention labels, and wired the generic Psionic server through that seam so `/v1/responses` continuation can survive process restarts without app-local storage glue. | `PSI-247`, `PSI-249` |
| `PSI-253` | Psionic Router: add MCP-aware tool loop execution boundary | `psionic-router`, `psionic-serve` | `SGLang` | Status: implemented on 2026-03-14 via GitHub issue `#3558`. Added a router-owned tool-loop controller with explicit per-step route, tool-call, tool-result, and continuation receipts; added native plus MCP-style provider descriptors with explicit history-visibility and result-visibility boundaries; and added a serve-side adapter test proving multi-step model -> tool -> continuation flow executes through Psionic-owned gateway logic instead of app-local orchestration. | `PSI-245`, `PSI-247`, `PSI-249` |

### Phase 5: Backend Closure And Validation

| Local ID | Proposed GitHub issue title | Scope | Primary reference | Description | Depends on |
| --- | --- | --- | --- | --- | --- |
| `PSI-254` | Psionic Local: finish portable local backend closure across CPU, Metal, CUDA, and hybrid offload | backend crates, `psionic-runtime`, `psionic-serve` | `llama.cpp` | Status: implemented on 2026-03-14 via GitHub issue `#3559`. Consolidated local-backend closure around the already-landed CPU, Metal, CUDA, and Apple GPT-OSS work: the generic CPU server plus the GPT-OSS server now expose aligned machine-checkable backend truth for residency, hybrid-offload posture, fallback policy, and performance class; the hardware validation matrix now explicitly separates positive execution rows from proxy/debug surfaces; and the older Apple Metal GPT-OSS chain is treated as absorbed history rather than an open blocker. | `PSI-235`, `METAL-GPT-OSS-1` through `METAL-GPT-OSS-7` |
| `PSI-255` | Psionic Cluster: add acceptance matrix for TP/PP/DP/EP plus PD execution across local and clustered serving | `psionic-cluster`, `psionic-runtime`, docs/tests | `vLLM` plus `SGLang` | Status: implemented on 2026-03-14 via GitHub issue `#3560`. Added `TOPOLOGY_ACCEPTANCE_MATRIX.md` plus `scripts/release/check-psionic-topology-acceptance-matrix.sh`, turning local and clustered topology claims into one executable acceptance contract with explicit supported, refused, and unsupported rows for replica-routed, pipeline-sharded, layer-sharded, tensor-sharded, PD modes, and expert-parallel non-support. | `PSI-240`, `PSI-242`, `PSI-249`, `PSI-250` |
| `PSI-256` | Psionic Benchmarks: define separate acceptance matrices for local portability, high-throughput serving, and structured agent workloads | docs/tests/benchmarks plus serving stack | mixed | Status: implemented on 2026-03-14 via GitHub issue `#3561`. Added `PRODUCT_CLASS_ACCEPTANCE_MATRICES.md` plus `scripts/release/check-psionic-product-class-acceptance.sh`, separating portability, throughput-serving, and structured-agent acceptance into distinct categories with explicit supported envelopes, refusal boundaries, and harness hooks instead of one blended benchmark story. | `PSI-254`, `PSI-255` |
| `PSI-257` | Psionic Pilot: validate one non-GPT-OSS family end to end through the generic server | `psionic-models`, `psionic-serve`, docs/tests | `llama.cpp` plus `vLLM` | Status: implemented on 2026-03-14 via GitHub issue `#3562`. Chose Qwen as the first explicit non-GPT-OSS pilot, added `NON_GPT_OSS_QWEN_PILOT.md` plus `scripts/release/check-psionic-qwen-pilot.sh`, and added a dedicated generic-server Qwen test that proves family inventory, scheduler headers, and scheduler receipts survive the full Psionic-owned runtime and generic-server path. | `PSI-233`, `PSI-234`, `PSI-256` |
| `PSI-258` | Psionic Pilot: validate one end-to-end agent workload with structured outputs, tool calls, response state, and cache-aware routing | `psionic-router`, `psionic-serve`, docs/tests | `SGLang` plus `vLLM` | Status: implemented on 2026-03-14 via GitHub issue `#3563`. Added `STRUCTURED_AGENT_WEATHER_PILOT.md` plus `scripts/release/check-psionic-weather-agent-pilot.sh`, and added one integrated weather-agent pilot test that combines structured JSON output, prefix-cache reuse, route headers, `/v1/responses` state, and router-owned tool-loop execution inside one end-to-end workload. | `PSI-244` through `PSI-253`, `PSI-256` |
