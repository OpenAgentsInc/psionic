...
| --- | --- | --- | --- |
| SPARTA-class sparse parameter averaging | deferred (side experiment only) | W3 standing order: no public gradients into the main optimizer, ever; sparse averaging runs only as a canary side experiment with its own evals and pre-written kill criteria | psionic#1127 (canary); Pluralis AsyncMesh `sparta/sparta.py` (read-only lane `projects/pluralis/repos/AsyncMesh/`) |
