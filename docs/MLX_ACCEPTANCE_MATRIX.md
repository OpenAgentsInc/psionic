...
> Status: canonical `PMLX-003` / `#3831` reference record, updated 2026-03-17
...
> refreshing the machine-readable report after `PMLX-601` / `#3866`,
> `PMLX-602` / `#3867`, `PMLX-603` / `#3868`, `PMLX-604` / `#3869`, and
> `PMLX-605` / `#3870` so the backend-closure category stays `planned` while
> the bounded CPU, Metal, CUDA, advanced operator-family, and seeded
> parity-harness slices are recorded explicitly.
...
| --- | --- | --- | --- | --- | --- |
| `array-runtime-surface` | `implemented_early` | Public lazy arrays exist with explicit evaluation, device and stream semantics, creation and view families, random or cast behavior, and host-materialization boundaries. | `psionic-array` now publishes a first user-facing lazy-array facade with runtime-backed device handles, honest unified-memory flags, explicit stream-dependency policy, graph-backed arithmetic, explicit `eval` / deferred `async_eval`, replay-stable eval receipts, scalar and filled-array creation helpers, `reshape` / `permute` / `transpose` / `flatten` / `expand_dims` / `squeeze` / `slice` / `select` / `concat` / `broadcast_to` view families, explicit seeded or best-effort random-uniform and random-normal creation, logical dtype casts, `arange` / `linspace` / `eye` helpers, axis-aware `sum` reduction, explicit host-owned typed buffer export, singleton `item()` extraction, deterministic tree flatten/map/unflatten utilities, and a bounded runtime resource report with explicit active/peak/cache counters plus cache-limit and reset controls above the reference eval substrate; it now also exposes fallible `ArrayContext::metal()` / `metal_seeded()` and `ArrayContext::cuda()` / `cuda_seeded()` constructors that route dense `constant` / `add` / `matmul` graphs through the selected runtime Metal or CUDA device while refusing broader public array materialization explicitly. | `PMLX-101` through `PMLX-106` | Treat this as supported early array/runtime closure only; it does not imply transform, `nn`, export, distributed MLX closure, or blanket Metal/CUDA execution parity beyond the bounded dense slices. |
...
| `backend-closure` | `planned` | CPU, Metal, and CUDA honestly cover the declared MLX surface, the parity harness carries upstream MLX test families, and compatibility or binding shells remain bounded. | Backend-specific substrate, the frozen MLX version window contract, a bounded CPU-reference coverage report, seeded parity-harness passes over imported `array_core`, `ops_numeric`, and `device_eval_memory` families, bounded public Metal plus CUDA execution slices, and a bounded advanced operator-family evidence layer now exist: `psionic-array` can create runtime-selected Metal and CUDA contexts and execute dense `constant` / `add` / `matmul` graphs with explicit device/stream receipts while keeping public accelerator numerics bounded to dense `f32`, `psionic-compat` now routes those imported families through `MlxParityHarnessReport` as seeded passes anchored to the CPU-reference oracle plus availability-aware accelerator hooks, `psionic-ir` now publishes reusable `linalg_gram_matrix`, `signal_naive_dft`, and `attention_rotary_residual_block` programs plus machine-readable `AdvancedOperatorProgramMatrixReport` evidence and explicit refusal for still-missing distribution and special-function families, and the optional `psionic-mlx-compat` plus `psionic-mlx-capi` shell now carries a repo-local migration guide and runnable examples so the compatibility boundary is explicit instead of implied; distributed backend-family capability mapping remains explicit in `psionic-distributed`, and broader backend closure still does not. | `PMLX-601` through `PMLX-607` plus `PMLX-004` and `PMLX-005` | Do not claim bounded MLX closure from one backend lane, one demo, or one local checkout. |
