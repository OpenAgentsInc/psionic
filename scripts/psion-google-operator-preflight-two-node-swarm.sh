...
PAIR_ID="${PAIR_ID:-}"
CLUSTER_NAMESPACE="${CLUSTER_NAMESPACE:-$(jq -r '.cluster_namespace' "${CONTRACT_FILE}")}"

usage() {
  cat <<'EOF'
Usage: psion-google-operator-preflight-two-node-swarm.sh [options]

Options:
  --pair <pair_id>                     Force one admitted zone pair.
  --cluster-namespace <namespace>      Override the cluster namespace to validate.
EOF
}
...
if [[ "${PROJECT_ID}" != "$(jq -r '.project_id' "${POLICY_FILE}")" ]]; then
  record_error "project_override" "unsupported project override ${PROJECT_ID}"
fi
if [[ "${CLUSTER_NAMESPACE}" != "$(jq -r '.expected_cluster_namespace' "${POLICY_FILE}")" ]]; then
  record_error "cluster_namespace" "cluster namespace ${CLUSTER_NAMESPACE} does not match the admitted configured-peer namespace"
fi

if [[ -n "${PAIR_ID}" ]]; then
  if ! jq -e --arg pair_id "${PAIR_ID}" '.supported_pairs[] | select(.pair_id == $pair_id)' "${POLICY_FILE}" >/dev/null; then
    record_error "pair_id" "unsupported zone pair ${PAIR_ID}"
  fi
fi
...
while IFS= read -r required_bucket_role; do
  if ! gcloud storage buckets get-iam-policy "${bucket_url}" --format=json | \
    jq -e \
    ...
    record_error "training_service_account_bucket_role" "${training_service_account} is missing required bucket role ${required_bucket_role} on ${bucket_url}"
  fi
done < <(jq -r '.required_bucket_roles[]' "${IDENTITY_PROFILE_FILE}")

network_name="$(jq -r '.network' "${NETWORK_POSTURE_FILE}")"
if ! gcloud compute networks describe "${network_name}" --project="${PROJECT_ID}" >/dev/null 2>&1; then
  record_error "network" "failed to describe network ${network_name}"
fi
while IFS= read -r subnetwork_name; do
  if ! gcloud compute networks subnets describe "${subnetwork_name}" \
    --project="${PROJECT_ID}" \
    --region="$(jq -r '.region' "${NETWORK_POSTURE_FILE}")" >/dev/null 2>&1; then
    record_error "subnetwork" "failed to describe subnetwork ${subnetwork_name}"
  fi
done < <(jq -r '.subnetworks[].subnetwork_name' "${NETWORK_POSTURE_FILE}")

if ! gcloud compute firewall-rules describe "$(jq -r '.ssh_posture.firewall_rule_name' "${NETWORK_POSTURE_FILE}")" --project="${PROJECT_ID}" >/dev/null 2>&1; then
  record_error "ssh_firewall_rule" "missing Google swarm IAP SSH firewall rule"
fi
if ! gcloud compute firewall-rules describe "$(jq -r '.cluster_transport_posture.firewall_rule_name' "${NETWORK_POSTURE_FILE}")" --project="${PROJECT_ID}" >/dev/null 2>&1; then
  record_error "cluster_firewall_rule" "missing Google swarm cluster firewall rule"
fi

quota_preflight_json=""
if quota_preflight_json="$(PROJECT_ID="${PROJECT_ID}" PAIR_ID="${PAIR_ID}" bash "${QUOTA_PREFLIGHT}" 2>/dev/null)"; then
  :
else
  quota_preflight_json="$(PROJECT_ID="${PROJECT_ID}" PAIR_ID="${PAIR_ID}" bash "${QUOTA_PREFLIGHT}" 2>/tmp/psion-google-two-node-swarm-quota.err || true)"
  record_error "quota_preflight" "quota preflight failed for pair ${PAIR_ID:-<auto>}"
fi
...
jq -n \
  --arg schema_version "psion.google_two_node_swarm_operator_preflight.v1" \
  ...
  --argjson supported_pairs "$(jq '.supported_pairs | map(.pair_id)' "${POLICY_FILE}")" \
  ...
  '{
    ...
    pair_override: (if $pair_id == "" then null else $pair_id end),
