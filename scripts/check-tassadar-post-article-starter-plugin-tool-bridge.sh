...
jq -e '
  ...
  and ((.projection_rows | length) == 6)
  and ((.execution_cases | length) == 6)
  and (.projection_rows | all(.stable_across_surfaces == true))
  and (.projection_rows | any(.tool_name == "plugin_text_url_extract"))
  and (.projection_rows | any(.tool_name == "plugin_text_stats"))
  and (.projection_rows | any(.tool_name == "plugin_http_fetch_text"))
  and (.projection_rows | any(.tool_name == "plugin_example_echo_guest"))
  ...
  and (.execution_cases | any(.case_id == "text_stats_success_bridge" and .plugin_id == "plugin.text.stats" and .status == "success"))
  and (.execution_cases | any(.case_id == "guest_echo_success_bridge" and .plugin_id == "plugin.example.echo_guest" and .status == "success"))
