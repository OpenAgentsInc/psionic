...
mod tassadar_compiled_distillation_targets;
mod tassadar_component_linking_profile;
mod tassadar_conditional_masking;
...
pub use tassadar_compiled_distillation_targets::*;
pub use tassadar_component_linking_profile::*;
pub use tassadar_conditional_masking::*;
...
impl DeterminismContractError {
    /// Returns the canonical refusal when one determinism-contract failure maps
    /// onto the shared Psionic refusal taxonomy.
    #[must_use]
    pub fn refusal(&self) -> PsionicRefusal {
        match self {
            Self::MissingGeneratorState { mode } => PsionicRefusal::new(
                PsionicRefusalCode::UnsupportedBackendCapability,
                PsionicRefusalScope::Runtime,
                self.to_string(),
            )
            .with_subject(format!("{mode:?}")),
            Self::InvalidDistributedRank { .. } => PsionicRefusal::new(
                PsionicRefusalCode::TopologyMismatch,
                PsionicRefusalScope::Topology,
                self.to_string(),
            )
            .with_subject("distributed_rank"),
        }
    }
}
