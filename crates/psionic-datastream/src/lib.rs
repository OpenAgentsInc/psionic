...
impl DatastreamCheckpointBinding {
    ...
    /// Creates a checkpoint binding for one Tassadar dynamic-memory resume artifact.
    #[must_use]
    pub fn tassadar_dynamic_memory_resume(checkpoint_id: impl AsRef<str>, step: u64) -> Self {
        let checkpoint_id = checkpoint_id.as_ref();
        Self::new("tassadar.dynamic_memory_resume.v1")
            .with_checkpoint_ref(format!(
    ...
    /// Creates a checkpoint binding for one Tassadar call-frame resume artifact.
    #[must_use]
    pub fn tassadar_call_frame_resume(checkpoint_id: impl AsRef<str>, step: u64) -> Self {
        let checkpoint_id = checkpoint_id.as_ref();
...
/// Explicit locator for one persisted Tassadar dynamic-memory resume artifact.
#[derive(Clone, Debug, PartialEq, Eq, Serialize, Deserialize)]
pub struct TassadarDynamicMemoryResumeLocator {
    /// Stable stream identifier.
    pub stream_id: String,
    /// Stable manifest digest.
    pub manifest_digest: String,
...
impl DatastreamManifestRef {
    ...
    /// Exports this manifest reference as a typed Tassadar dynamic-memory resume locator.
    pub fn tassadar_dynamic_memory_resume_locator(
        &self,
    ) -> Result<TassadarDynamicMemoryResumeLocator, DatastreamTransferError> {
        let checkpoint_binding = self.checkpoint_binding.as_ref().ok_or_else(|| {
            DatastreamTransferError::TassadarDynamicMemoryResumeContractInvalid {
                stream_id: self.stream_id.clone(),
                subject: self.subject,
                checkpoint_family: None,
            }
        ...
        if self.subject != DatastreamSubjectKind::Checkpoint
            || !checkpoint_binding
                .checkpoint_family
                .starts_with(TASSADAR_DYNAMIC_MEMORY_RESUME_FAMILY_PREFIX)
        {
            return Err(
                DatastreamTransferError::TassadarDynamicMemoryResumeContractInvalid {
                    stream_id: self.stream_id.clone(),
        ...
        Ok(TassadarDynamicMemoryResumeLocator {
            stream_id: self.stream_id.clone(),
            manifest_digest: self.manifest_digest.clone(),
            checkpoint_ref: checkpoint_binding
                .checkpoint_ref
            ...
            total_bytes: self.total_bytes,
            detail: format!(
                "tassadar dynamic-memory resume locator via family `{}` ref `{}`",
                checkpoint_binding.checkpoint_family,
                checkpoint_binding
                    .checkpoint_ref
                    .as_deref()
    ...
    pub fn tassadar_call_frame_resume_locator(
        &self,
    ) -> Result<TassadarCallFrameResumeLocator, DatastreamTransferError> {
        let checkpoint_binding = self.checkpoint_binding.as_ref().ok_or_else(|| {
...
pub enum DatastreamTransferError {
    ...
    /// The manifest reference is not a valid Tassadar dynamic-memory resume contract.
    #[error(
    ...
    /// The manifest reference is not a valid Tassadar call-frame resume contract.
    #[error(
...
mod tests {
    ...
    #[test]
    fn checkpoint_manifest_can_export_tassadar_dynamic_memory_resume_locator()
    -> Result<(), Box<dyn std::error::Error>> {
        let payload = br#"{"checkpoint_id":"copy_fill"}"#.to_vec();
        let manifest = super::DatastreamManifest::from_bytes(
            "tassadar-dynamic-memory-4",
            DatastreamSubjectKind::Checkpoint,
            &payload,
            8,
            DatastreamEncoding::RawBinary,
        ...
        .with_checkpoint_binding(
            DatastreamCheckpointBinding::tassadar_dynamic_memory_resume("copy_fill", 4),
        );

        let locator = manifest
            .manifest_ref()
            .tassadar_dynamic_memory_resume_locator()?;
        assert_eq!(locator.stream_id, "tassadar-dynamic-memory-4");
        assert_eq!(locator.manifest_digest, manifest.stable_digest());
        assert_eq!(
            locator.checkpoint_ref,
            "checkpoint://tassadar.dynamic_memory_resume/copy_fill"
        );
        assert_eq!(locator.step, Some(4));
        assert_eq!(locator.object_digest, manifest.object_digest);
        Ok(())
    }

    #[test]
    fn non_dynamic_memory_checkpoint_manifest_is_refused_as_dynamic_memory_resume_locator() {
        let manifest = super::DatastreamManifest::from_bytes(
            "checkpoint-train-19",
            DatastreamSubjectKind::Checkpoint,
            b"weights",
        ...
        .with_checkpoint_binding(
            DatastreamCheckpointBinding::tassadar_execution_checkpoint("alpha", 19),
        );

        let error = manifest
            .manifest_ref()
            .tassadar_dynamic_memory_resume_locator()
            .expect_err("non-dynamic-memory checkpoint manifest should be refused");
        assert_eq!(
            error,
            DatastreamTransferError::TassadarDynamicMemoryResumeContractInvalid {
    ...
    #[test]
    fn checkpoint_manifest_can_export_tassadar_call_frame_resume_locator()
    -> Result<(), Box<dyn std::error::Error>> {
        let payload = br#"{"checkpoint_id":"recursive_sum"}"#.to_vec();
        let manifest = super::DatastreamManifest::from_bytes(
            "tassadar-call-frame-5",
            DatastreamSubjectKind::Checkpoint,
            &payload,
            8,
            DatastreamEncoding::RawBinary,
        ...
        assert_eq!(
            locator.checkpoint_ref,
            "checkpoint://tassadar.call_frame_resume/recursive_sum"
        );
    ...
    #[test]
    fn non_call_frame_checkpoint_manifest_is_refused_as_call_frame_resume_locator() {
        let manifest = super::DatastreamManifest::from_bytes(
            "checkpoint-train-23",
            DatastreamSubjectKind::Checkpoint,
            b"weights",
        ...
        .with_checkpoint_binding(
            DatastreamCheckpointBinding::tassadar_execution_checkpoint("alpha", 23),
        );

        let error = manifest
            .manifest_ref()
            .tassadar_call_frame_resume_locator()
            .expect_err("non-call-frame checkpoint manifest should be refused");
        assert_eq!(
            error,
            DatastreamTransferError::TassadarCallFrameResumeContractInvalid {
