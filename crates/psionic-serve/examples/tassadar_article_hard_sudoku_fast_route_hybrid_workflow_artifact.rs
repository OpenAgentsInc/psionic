...
const TASSADAR_ARTICLE_HARD_SUDOKU_FAST_ROUTE_HYBRID_WORKFLOW_ARTIFACT_REF: &str =
    "fixtures/tassadar/reports/tassadar_article_hard_sudoku_fast_route_hybrid_workflow_artifact.json";

#[derive(Serialize)]
struct TassadarArticleHardSudokuFastRouteHybridWorkflowArtifactCase {
    name: String,
...
#[derive(Serialize)]
struct TassadarArticleHardSudokuFastRouteHybridWorkflowArtifact {
    schema_version: u16,
    benchmark_report_ref: String,
    cases: Vec<TassadarArticleHardSudokuFastRouteHybridWorkflowArtifactCase>,
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let cases = vec![
        collect_case(
            &service,
            "delegated_sudoku_9x9_test_a_hull",
            TassadarArticleHybridWorkflowRequest::new(
                "article-hard-sudoku-fast-route-hybrid-test-a",
                "planner-session-article-hard-sudoku-test-a",
                "planner-article-fixture-v0",
                "workflow-step-hard-sudoku-test-a",
                "delegate exact hard-sudoku stand-in article workload into Tassadar",
                "sudoku_9x9_test_a",
        ...
        collect_case(
            &service,
            "delegated_arto_inkala_hull",
            TassadarArticleHybridWorkflowRequest::new(
                "article-hard-sudoku-fast-route-hybrid-arto",
                "planner-session-article-hard-sudoku-arto",
                "planner-article-fixture-v0",
                "workflow-step-hard-sudoku-arto",
                "delegate named Arto Inkala hard-sudoku article workload into Tassadar",
    ...
    let mut artifact = TassadarArticleHardSudokuFastRouteHybridWorkflowArtifact {
        schema_version: 1,
    ...
    let path = Path::new(TASSADAR_ARTICLE_HARD_SUDOKU_FAST_ROUTE_HYBRID_WORKFLOW_ARTIFACT_REF);
    ...
    println!(
        "wrote {} ({})",
        TASSADAR_ARTICLE_HARD_SUDOKU_FAST_ROUTE_HYBRID_WORKFLOW_ARTIFACT_REF,
...
) -> Result<TassadarArticleHardSudokuFastRouteHybridWorkflowArtifactCase, Box<dyn std::error::Error>>
{
    let outcome = service.execute(&request)?;
    Ok(
        TassadarArticleHardSudokuFastRouteHybridWorkflowArtifactCase {
...
fn stable_digest<T: Serialize>(value: &T) -> String {
    ...
    hasher.update(b"psionic_tassadar_article_hard_sudoku_fast_route_hybrid_workflow_artifact|");
