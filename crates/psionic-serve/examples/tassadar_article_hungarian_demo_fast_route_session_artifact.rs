...
const TASSADAR_ARTICLE_HUNGARIAN_DEMO_FAST_ROUTE_SESSION_ARTIFACT_REF: &str =
    "fixtures/tassadar/reports/tassadar_article_hungarian_demo_fast_route_session_artifact.json";

#[derive(Serialize)]
struct TassadarArticleHungarianDemoFastRouteSessionArtifactCase {
    name: String,
...
#[derive(Serialize)]
struct TassadarArticleHungarianDemoFastRouteSessionArtifact {
    schema_version: u16,
    benchmark_report_ref: String,
    cases: Vec<TassadarArticleHungarianDemoFastRouteSessionArtifactCase>,
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let cases = vec![collect_case(
        &service,
        "direct_hungarian_10x10_hull",
        TassadarArticleExecutorSessionRequest::new(
            "article-hungarian-demo-fast-route-session",
            "hungarian_10x10_test_a",
            psionic_runtime::TassadarExecutorDecodeMode::HullCache,
        ),
    )?];

    let mut artifact = TassadarArticleHungarianDemoFastRouteSessionArtifact {
        schema_version: 1,
    ...
    let path = Path::new(TASSADAR_ARTICLE_HUNGARIAN_DEMO_FAST_ROUTE_SESSION_ARTIFACT_REF);
    ...
    println!(
        "wrote {} ({})",
        TASSADAR_ARTICLE_HUNGARIAN_DEMO_FAST_ROUTE_SESSION_ARTIFACT_REF, artifact.artifact_digest
    );
    Ok(())
}
...
) -> Result<TassadarArticleHungarianDemoFastRouteSessionArtifactCase, Box<dyn std::error::Error>> {
    let outcome = service.execute(&request)?;
    Ok(TassadarArticleHungarianDemoFastRouteSessionArtifactCase {
...
fn stable_digest<T: Serialize>(value: &T) -> String {
    ...
    hasher.update(b"psionic_tassadar_article_hungarian_demo_fast_route_session_artifact|");
