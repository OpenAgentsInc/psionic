...
impl GenerateConformanceCase {
    ...
    pub fn from_generate_compatible_prompt_fixture(
        ...
        for (index, message) in render_case.messages.iter().enumerate() {
            match message.role {
                GoldenPromptRole::System | GoldenPromptRole::Developer
                    if index == 0 && system.is_none() =>
                {
                ...
                _ => {
                    return Err(ConformanceSubjectError::InvalidCase {
                        case_id: render_case_id.to_string(),
                        message: String::from(
                            "only single-turn instruction-plus-user fixture cases can be mapped to /api/generate today",
                        ),
                    });
                }
            }
...
mod tests {
    ...
    -> Result<(), Box<dyn std::error::Error>> {
        ...
        let suite = ConformanceSuite {
            id: String::from("fixture-gemma4-suite"),
            compare_tags: false,
            compare_ps: false,
            show_cases: Vec::new(),
            generate_cases: vec![case],
        ...
        let baseline = RecordedConformanceSubject::new("gemma4-baseline").with_generate_case(
            "gemma4-render",
            SubjectObservation::Supported(GenerateObservation {
                rendered_prompt: Some(expected_rendered_prompt.clone()),
                output_text: String::new(),
