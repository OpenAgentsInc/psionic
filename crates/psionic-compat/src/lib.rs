...
impl MlxParityHarnessReport {
    ...
    fn from_selected_families(
        ...
        let schema_path = String::from("docs/mlx_parity_harness_report.schema.json");
    ...
    MlxAcceptanceMatrixReport::new(vec![
        MlxAcceptanceCategory {
            ...
            current_repo_truth: String::from(
                "Psionic now publishes a first user-facing lazy-array facade in psionic-array with runtime-backed device handles, honest unified-memory flags, explicit stream-dependency policy, graph-backed arithmetic, explicit eval and deferred async_eval, replay-stable eval receipts, scalar and filled-array creation helpers, reshape/permute/transpose/flatten/expand_dims/squeeze/slice/select/concat/broadcast_to view families, explicit seeded or best-effort random-uniform and random-normal creation, logical dtype casts, arange/linspace/eye helpers, axis-aware sum reduction, explicit host-owned typed buffer export, singleton item extraction, deterministic tree flatten/map/unflatten utilities, and a bounded runtime resource report with explicit active/peak/cache counters plus cache-limit and reset controls above the reference eval substrate; it now also exposes fallible ArrayContext::metal and metal_seeded plus ArrayContext::cuda and cuda_seeded constructors that route dense constant/add/matmul graphs through the selected runtime Metal or CUDA device while refusing broader public array materialization explicitly.",
            ),
            boundary_note: String::from(
                "Treat this as supported early array/runtime closure only; it does not imply transform, nn, export, distributed MLX closure, or blanket Metal/CUDA execution parity beyond the bounded dense slices.",
            ),
        },
        MlxAcceptanceCategory {
    ...
    MlxParityHarnessReport::new(vec![
        MlxParityHarnessFamily {
            ...
            psionic_hook_commands: vec![
                ...
                String::from(
                    "cargo test -p psionic-array tests::public_lazy_array_cuda_eval_refuses_ops_outside_bounded_surface_when_hardware_available -- --exact --nocapture",
                ),
            ],
            summary: String::from(
        ...
        MlxParityHarnessFamily {
            ...
            psionic_hook_commands: vec![
                ...
                String::from(
                    "cargo test -p psionic-array tests::public_lazy_array_cuda_eval_executes_bounded_dense_surface_when_hardware_available -- --exact --nocapture",
                ),
            ],
            summary: String::from(
        ...
        MlxParityHarnessFamily {
            ...
            psionic_hook_commands: vec![
                ...
                String::from(
                    "cargo test -p psionic-array tests::public_lazy_array_runtime_resource_report_tracks_active_peak_and_cache_counters -- --exact --nocapture",
                ),
                String::from(
                ...
                String::from(
                    "cargo test -p psionic-array tests::public_lazy_array_cuda_constructor_reports_runtime_availability -- --exact --nocapture",
                ),
                String::from(
                    "cargo test -p psionic-array tests::public_lazy_array_metal_eval_executes_bounded_dense_surface_when_hardware_available -- --exact --nocapture",
                ),
        ...
        MlxParityHarnessFamily {
            ...
            psionic_hook_commands: vec![
                ...
                String::from(
                    "cargo test -p psionic-distributed tests::parse_hostfile_accepts_comments_slots_and_addresses -- --exact --nocapture",
                ),
                String::from(
                ...
                String::from(
                    "cargo test -p psionic-distributed tests::grouped_all_sum_preserves_tree_structure_and_reduces_grouped_leaves -- --exact --nocapture",
                ),
                String::from(
                    "cargo test -p psionic-distributed tests::average_gradients_divides_grouped_reduction_by_world_size -- --exact --nocapture",
                ),
                String::from(
                    "cargo test -p psionic-distributed tests::all_to_sharded_linear_slices_output_rows_into_local_wrappers -- --exact --nocapture",
                ),
                String::from(
                    "cargo test -p psionic-distributed tests::sharded_to_all_linear_reconstructs_full_output_from_rank_wrappers -- --exact --nocapture",
                ),
                String::from(
                    "cargo test -p psionic-distributed tests::tensor_parallel_wrappers_refuse_missing_rank_state_and_tiny_axes -- --exact --nocapture",
                ),
                String::from(
                    "cargo test -p psionic-distributed tests::fsdp_apply_gradients_updates_mixed_full_shard_and_replicated_groups -- --exact --nocapture",
                ),
                String::from(
                    "cargo test -p psionic-distributed tests::fsdp_apply_gradients_applies_global_norm_clip_before_local_update -- --exact --nocapture",
                ),
                String::from(
                    "cargo test -p psionic-distributed tests::fsdp_apply_gradients_refuses_invalid_remote_maps_and_unsupported_layouts -- --exact --nocapture",
                ),
            ],
            summary: String::from(
    ...
    MlxCompatibilityMatrixReport::new(vec![
        ...
        MlxCompatibilityMatrixEntry {
            ...
            evidence_refs: vec![
                ...
                String::from("ArrayMemoryCounters"),
            ...
            blocking_issue_refs: Vec::new(),
            boundary_note: String::from(
                "This is a bounded supported early array surface; it does not imply blanket MLX Metal or CUDA parity, MLX transform, nn, native function export, distributed support, or generic custom-op runtime execution.",
            ),
        },
        MlxCompatibilityMatrixEntry {
        ...
        MlxCompatibilityMatrixEntry {
            ...
            matrix_status: MlxCompatibilityMatrixStatus::Convertible,
            summary: String::from(
                "Bounded local MLX-style text, catalog, text-serving, multimodal, and audio packages now exist in `psionic-mlx-lm`, `psionic-mlx-catalog`, `psionic-mlx-serve`, `psionic-mlx-vlm`, and `psionic-mlx-audio`, but recipe layers, synthetic/publish workflows, and benchmark packages remain later ecosystem work.",
            ),
            evidence_refs: vec![
            ...
            boundary_note: String::from(
                "The first local text, catalog, text-serving, multimodal, and audio packages are real, but they do not imply training-recipe closure, synthetic/publish workflows, or benchmark-package completion.",
            ),
        },
    ])
}
    ...
    -> Result<(), Box<dyn std::error::Error>> {
        ...
        assert!(
            ecosystem
                .blocking_issue_refs
                .iter()
                .all(|issue| !issue.contains("PMLX-705"))
        );
        assert!(
            ecosystem
                .blocking_issue_refs
                .iter()
                .all(|issue| !issue.contains("PMLX-702"))
        );
