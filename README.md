...
- Probe GEPA rollout coordinator: [docs/PROBE_GEPA_ROLLOUT_COORDINATOR.md](docs/PROBE_GEPA_ROLLOUT_COORDINATOR.md)
...
- Inference and local serving
  ...
  - CSM speech-generation lane for Autopilot, currently at the Rust-only warm
    CPU worker stage with committed parity fixtures plus governed operator
    voice
    profile admission, Rust tokenizer, prompt-framing, config,
    artifact-descriptor, Rust CPU CSM generation, Rust Mimi decode, PCM16-WAV,
    buffered multipart chunks, worker metadata, request/cancellation/timeout
  ...
  - bounded non-`GptOss` lanes including `qwen35`, the planned-execution
    `qwen38` lane with implemented identity, prompt/tokenizer, and BF16 header
    admission tracked in [docs/qwen38/README.md](docs/qwen38/README.md), the published dense
